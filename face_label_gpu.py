...
if os.name == "nt":
    def _auto_add_nvidia_bins(prefix):
        # Look for .../site-packages/nvidia/*/bin
        nvidia_root = os.path.join(prefix, "Lib", "site-packages", "nvidia")
        pattern = os.path.join(nvidia_root, "*", "bin")
        
        for bin_path in glob.glob(pattern):
            if os.path.isdir(bin_path):
                os.add_dll_directory(bin_path)
                # print(f"Added: {bin_path}") # Uncomment to verify

    _auto_add_nvidia_bins(sys.prefix)       # Check venv
    _auto_add_nvidia_bins(sys.base_prefix)  # Check base python
import warnings, argparse, csv, json
import cv2
import numpy as np
...
# Hint InsightFace/ONNXRuntime to prefer CUDA, then CPU
...
def init_face_app(det_size=(640, 640), use_gpu=True):
    av = set(ort.get_available_providers())
    want_gpu = use_gpu and ("CUDAExecutionProvider" in av)
    app = FaceAnalysis(name='buffalo_l')
    # Older InsightFace may not accept 'providers' kwarg; rely on env + ctx_id
    app.prepare(ctx_id=(0 if want_gpu else -1), det_size=det_size)
    return app, want_gpu
    ...
    for vid_idx, path in enumerate(video_paths):
        ...
        while True:
            ...
            for f in faces:
                ...
                if score < min_det_score: continue
...
def main():
    p = argparse.ArgumentParser(description="GPU-accelerated face labeling (CUDA if available), with CPU fallback")
    ...
    p.add_argument('--min_det_score', type=float, default=0.4, help='Min detector score for clustering samples')
    ...
    p.add_argument('--cpu_only', action='store_true', help='Force CPU even if CUDA is available')
    args = p.parse_args()
    ...
    app, used_gpu = init_face_app(use_gpu=not args.cpu_only)
